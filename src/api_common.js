...
ecc.randomKey().then(privateKey => {
  ...
  console.log('Public Key:\t', ecc.privateToPublic(privateKey)) // VKTkey...
    ...
    */
    privateToPublic: (wif, pubkey_prefix = 'VKT') =>
      PrivateKey(wif).toPublic().toString(pubkey_prefix),

    /**
        @arg {pubkey} pubkey - like VKTKey..
        @arg {string} [pubkey_prefix = 'VKT']

        @return {boolean} valid

        @example ecc.isValidPublic(pubkey) === true
    */
    isValidPublic: (pubkey, pubkey_prefix = 'VKT') =>
    ...
    */
    ...
    /**
        ...
        @arg {String|Buffer} signature (VKTbase58sig.., Hex, Buffer)
